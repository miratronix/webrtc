...
func (m *MediaEngine) updatePayloadTypes(supportedCodecs map[int]*RTCRtpCodec) error {
	for _, codec := range m.codecs {
		for _, supported := range supportedCodecs {
			if codec.equals(supported) {
				codec.PayloadType = supported.PayloadType
			}
		}
		if codec.PayloadType == 0 {
			return errors.New("Unsupported codec " + codec.Name)
		}
	}
	return nil
}

func (m *MediaEngine) getCodec(name string) (*RTCRtpCodec, error) {
	for _, codec := range m.codecs {
		if codec.Name == name {
			return codec, nil
		}
...
func (c *RTCRtpCodec) equals(other *RTCRtpCodec) bool {
	return strings.ToLower(c.Name) == strings.ToLower(other.Name) &&
		c.ClockRate == other.ClockRate &&
		c.Channels == other.Channels &&
		c.SdpFmtpLine == other.SdpFmtpLine
}
